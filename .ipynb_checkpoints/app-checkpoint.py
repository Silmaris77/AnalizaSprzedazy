...
class db:
    def __init__(self):
        self.transactions = self.transaction_init()  # Poprawione wywołanie metody
    ...
    def merge(self):
        ...
        # Merging customer data with country codes
        df = df.join(
...
# Tworzenie obiektu klasy db i wywołanie metody merge
df = db()
df.merge()
...
@app.callback(Output('sales-weekdays', 'figure'),
              [Input('sales-range', 'start_date'), Input('sales-range', 'end_date')])
def render_sales_weekdays(start_date, end_date):
    return tab3.sales_weekdays_fig(df.merged, start_date, end_date)

@app.callback(Output('customer-analysis', 'figure'),
              [Input('sales-range', 'start_date'), Input('sales-range', 'end_date')])
def render_customer_analysis(start_date, end_date):
    return tab3.customer_analysis_fig(df.merged, start_date, end_date)
